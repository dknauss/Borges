...
- The External Services sections of `readme.txt` and `README.md`, which WordPress.org requires, now disclose the arXiv API, the Open Library Books API, and the Google Books API, and note that PMCID lookups use the same NCBI exporter as PMID. They previously listed only Crossref and PubMed.
